...
> ⚠️ **Superseded by [`docs/three-dots-gameplay-script.md`](./three-dots-gameplay-script.md)**
> (2026-08-18). Kept for history. This early script was built around a **countdown timer +
> game-over**, which the shipped **Endless** mode does NOT have (Endless is zen — no fail state,
> no timer, deadlock reshuffles). It also predates the shipped **8-way** adjacency and the **≥3**
> chain / **≥5-line** sweep rules. Read the three-dots script for current behavior; matches the
> `two-dots-game-design.md → three-dots-game-design.md` supersession precedent.

**Date:** 2026-06-18
**Status:** Superseded — historical reference only. Mirrors `docs/two-dots-storyboard.html`.
