...
| **Core · obstacles**  | `src/core/obstacles/`                       | Yes                    | Obstacle behavior; layered-cage state (`Map<CellIndex, number>` = index→layers) + single-hit anchor overlay (`Set<CellIndex>`)                                          | `caged-dot.ts` (`protectedOf` = cages with ≥2 layers; `chipLayers` peel), `anchor.ts` (`removeAdjacent` = the one 8-way removal rule; `buildAnchors`/`dropAnchors`/`remapAnchors`/`toMask`)                                                                                                                                                                                                 |
| **Render**            | `src/render/`                               | No (Skia)              | Draw board, dots, active link path; cage overlay (layer pips, chip rattle); anchor weight overlay                                                                       | `board-canvas.tsx`, `dot-layer.tsx`, `link-path.tsx`, `cage-overlay-layer.tsx`, `cage-intro-popup.tsx`, `anchor-overlay-layer.tsx`, `palette.ts` (`DOT_COLORS`, append-only), `ui-theme.ts` (paper-craft chrome tokens for the `src/app` meta screens; reuses palette's dark ground), `rive-title.tsx` (`<RiveTitle>` — Rive-driven title, falls back to the static lockup; see note below) |
...
> **Note (2026-08-27) — anchor / weight obstacle.** A second obstacle overlay: a `Set<CellIndex>` of
> single-hit paper weights, threaded by both `use-journey-state.ts` and `use-voyage-state.ts` alongside
> the cage map. It reuses the same overlay-blind colour core via a generic extension of `resolve-chain.ts`
> — an optional `{ skipCollect, expandCleared }` seam argument. `skipCollect` keeps an anchor cell out of a
> colour-sweep's collection; `expandCleared` maps the scored `cleared` set to the extra cells to empty,
> echoed on `Resolution.expandedCleared`. `src/core/resolve-anchor-chain.ts` is the one bridge both mode
> hooks and the solver call — it composes the cage `protectedCells` and the anchor seam over the colour
> core in a single pass. The removal rule (`removeAdjacent`, `src/core/obstacles/anchor.ts`: an anchor is
> removed iff a cleared cell is 8-way adjacent to it — any colour, single-hit, no layers) is shared verbatim
> by the runtime fold and the solver, so a level that passes the winnability sweep is beatable with the real
> removal mechanic. A cell can never be both caged and anchored (the schema dedups obstacles by cell), so the
> two overlays are disjoint and echo back independently (`protectedHits` / `expandedCleared`). Render:
> `src/render/anchor-overlay-layer.tsx` — a sibling `pointerEvents="none"` overlay, gravity-follow via the
> shared board-animation offsets (the cage-overlay discipline). Endless never uses obstacles. Objective:
> `clearAnchors` (remove every weight). Verified on the RN-free core; the overlay layer is on-device-verified
> like every other render layer.
